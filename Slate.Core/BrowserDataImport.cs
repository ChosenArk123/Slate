...
public enum CredentialImportDisposition { Add, Update, Unchanged }

public sealed record CredentialImportReviewItem(ImportedCredential Credential, CredentialImportDisposition Disposition, CredentialMetadata? Existing);

public sealed class CredentialImportReview(BrowserDataImportPackage package, IReadOnlyList<CredentialImportReviewItem> items) : IDisposable
{
    public BrowserDataImportPackage Package { get; } = package;
    public IReadOnlyList<CredentialImportReviewItem> Items { get; } = items;
    public int NewCount => Items.Count(item => item.Disposition == CredentialImportDisposition.Add);
    public int ChangedCount => Items.Count(item => item.Disposition == CredentialImportDisposition.Update);
    public int UnchangedCount => Items.Count(item => item.Disposition == CredentialImportDisposition.Unchanged);
    public void Dispose() => Package.Dispose();
}

public sealed record CredentialImportResult(int Added, int Updated, int Unchanged, int ChangedSkipped, int Conflicts)
{
    public BrowserDataImportResult ToImportResult(BrowserDataImportPackage package, bool replaceChanged)
    {
        ArgumentNullException.ThrowIfNull(package);
        int imported = Added + (replaceChanged ? Updated : 0);
        int skipped = Unchanged + (!replaceChanged ? ChangedSkipped : 0);
        int duplicates = package.DuplicateRows;
        int conflicts = package.ConflictingRows + Conflicts;
        int invalid = package.RejectedRows;
        int failed = 0;
        int totalRows = package.Credentials.Count + duplicates + package.ConflictingRows + invalid;
        return new BrowserDataImportResult(
            totalRows,
            imported,
            skipped,
            ...
            invalid,
            failed,
            package.AvailableKinds,
            package.SourceName);
    }
}

/// <summary>Reviews and applies imported credentials through the vault's normal optimistic write path.</summary>
public static class CredentialImportCoordinator
{
    /// <summary>
    /// Imports validated password records without overwriting an existing credential. Each accepted record
    /// uses the vault's ordinary assessment and revision-checked save path so one failed write does not hide
    /// successful records before or after it.
    /// </summary>
    public static async Task<BrowserDataImportResult> ImportAsync(
        CredentialVault vault, BrowserDataImportPackage package, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(vault);
        ArgumentNullException.ThrowIfNull(package);
        if (!package.AvailableKinds.HasFlag(BrowserDataKinds.Passwords))
            throw new BrowserDataImportException("The selected import does not contain passwords.");

        int imported = 0;
        int duplicates = package.DuplicateRows;
        int conflicts = package.ConflictingRows;
        int invalid = package.RejectedRows;
        int failed = 0;

        foreach (var credential in package.Credentials)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!CredentialVault.IsValidCredential(credential.Origin, credential.Username, credential.Password))
            {
                invalid++;
                continue;
            }
            ...
            try
            {
                var decision = await vault.AssessAsync(
                    credential.Origin, credential.Username, credential.Password).ConfigureAwait(false);
                if (decision.Change == CredentialChange.Unchanged)
                {
                    duplicates++;
                    continue;
                }
                if (decision.Change == CredentialChange.Update)
                {
                    conflicts++;
                    continue;
                }
                ...
                try
                {
                    await vault.SaveAsync(credential.Origin, credential.Username, credential.Password, expected: null)
                        .ConfigureAwait(false);
                    imported++;
                }
                catch (CredentialConflictException)
                {
                    // Another writer created this identity after assessment. Reclassify without overwriting it.
                    try
                    {
                        var current = await vault.AssessAsync(
                            credential.Origin, credential.Username, credential.Password).ConfigureAwait(false);
                        if (current.Change == CredentialChange.Unchanged) duplicates++;
                        else conflicts++;
                    }
        ...
        int total = checked(imported + duplicates + conflicts + invalid + failed);
        return new BrowserDataImportResult(total, imported, skipped: 0, duplicates, conflicts, invalid, failed,
            BrowserDataKinds.Passwords, package.SourceName);
    }

    public static async Task<CredentialImportReview> ReviewAsync(CredentialVault vault, BrowserDataImportPackage package)
    {
        ArgumentNullException.ThrowIfNull(vault);
        ArgumentNullException.ThrowIfNull(package);
        if (!package.AvailableKinds.HasFlag(BrowserDataKinds.Passwords))
            throw new BrowserDataImportException("The selected import does not contain passwords.");
        int existingCount = (await vault.ListAsync().ConfigureAwait(false)).Count;
        var decisions = await vault.AssessBatchAsync(package.Credentials.Select(credential =>
            new CredentialAssessmentRequest(credential.Origin, credential.Username, credential.Password))).ConfigureAwait(false);
        var items = new List<CredentialImportReviewItem>(package.Credentials.Count);
        for (int index = 0; index < package.Credentials.Count; index++)
        {
            var credential = package.Credentials[index];
            var decision = decisions[index];
            items.Add(new CredentialImportReviewItem(credential, decision.Change switch
            {
                CredentialChange.Save => CredentialImportDisposition.Add,
                CredentialChange.Update => CredentialImportDisposition.Update,
                _ => CredentialImportDisposition.Unchanged
            }, decision.Existing));
        }
        if (existingCount + items.Count(item => item.Disposition == CredentialImportDisposition.Add) > CredentialVault.MaximumEntries)
            throw new BrowserDataImportException("The imported passwords would exceed the vault capacity.");
        return new CredentialImportReview(package, items);
    }

    public static async Task<CredentialImportResult> ApplyAsync(CredentialVault vault, CredentialImportReview review, bool replaceChanged)
    {
        ArgumentNullException.ThrowIfNull(vault);
        ArgumentNullException.ThrowIfNull(review);
        int unchanged = review.UnchangedCount;
        int skipped = replaceChanged ? 0 : review.ChangedCount;
        var selected = review.Items.Where(item => item.Disposition == CredentialImportDisposition.Add ||
            replaceChanged && item.Disposition == CredentialImportDisposition.Update).ToArray();
        if (selected.Length == 0) return new CredentialImportResult(0, 0, unchanged, skipped, 0);
        var requests = selected.Select(item => new CredentialSaveRequest(
            item.Credential.Origin, item.Credential.Username, item.Credential.Password, item.Existing)).ToArray();
        try
        {
            await vault.SaveBatchAsync(requests).ConfigureAwait(false);
            return new CredentialImportResult(
                selected.Count(item => item.Disposition == CredentialImportDisposition.Add),
                selected.Count(item => item.Disposition == CredentialImportDisposition.Update),
                unchanged, skipped, 0);
        }
        catch (CredentialConflictException)
        {
            return new CredentialImportResult(0, 0, unchanged, skipped, selected.Length);
        }
    }

    public static async Task<BrowserDataImportResult> ApplyImportAsync(CredentialVault vault, CredentialImportReview review, bool replaceChanged)
    {
        var raw = await ApplyAsync(vault, review, replaceChanged).ConfigureAwait(false);
        return raw.ToImportResult(review.Package, replaceChanged);
    }
}
