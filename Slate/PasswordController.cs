...
internal sealed class PasswordController : IDisposable
{
    ...
    private readonly Func<bool> _live, _temporary, _alive, _autofill, _private;
    ...
    public PasswordController(CoreWebView2 core, CredentialVault vault, Func<bool> live, Func<bool> temporary,
        Func<bool>? alive = null, Func<bool>? autofill = null, Func<bool>? privateContext = null,
        Func<string?>? pageUrl = null)
    {
        _core = core; _vault = vault; _live = live; _temporary = temporary; _alive = alive ?? live;
        _autofill = autofill ?? (() => false); _private = privateContext ?? (() => false);
    ...
    private async Task AttachDocumentAsync()
    {
        ...
        if (_autofill() && !_temporary() && Navigation.IsSecureOrigin(ticket!.Origin)) await AutofillIfUnambiguousAsync(ticket);
    }
    ...
    private async Task AutofillIfUnambiguousAsync(Ticket ticket)
    {
        try
        {
            var accounts = await AccountsAsync(ticket);
        ...
        catch { /* Vault or document failures leave the page untouched. */ }
    }
    ...
    {
        if (!Ready || _temporary() || _private() || _candidate is not null) return;
        try
        {
    ...
    {
        if (_candidate is { } expired && (expired.Expires < DateTimeOffset.UtcNow || !_alive() || _temporary() || _private())) { Dismiss(); return; }
        if (_busy) return;
        ...
        try
        {
            ...
            if (!Valid(ticket) || _candidate != candidate || _temporary() || _private()) return;
    ...
    public async Task<bool> SaveOfferAsync()
    {
        ...
        if (candidate?.Decision is null || !_live() || _temporary() || _private() || candidate.Expires < DateTimeOffset.UtcNow ||
            CredentialOrigin.Normalize(_core.Source) != candidate.Origin) return false;
