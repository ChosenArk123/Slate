...
public sealed class BrowserCommandRouter : IDisposable
{
    ...
    private const int VK_RMENU = 0xA5; // Right Alt; Windows uses Ctrl+Right Alt for AltGr.
    ...
    private IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
    {
        if (!_disposed && nCode >= 0)
        {
            ...
            if (msg is WM_KEYDOWN or WM_SYSKEYDOWN)
            {
                if (IsSlateForeground())
                {
                    ...
                    if ((GetAsyncKeyState(VK_LWIN) & 0x8000) == 0 && (GetAsyncKeyState(VK_RWIN) & 0x8000) == 0)
                    {
                        ...
                        // AltGr is reported by Windows as Ctrl+Right Alt. It must remain text input,
                        // even if the resulting virtual-key tuple resembles a Slate shortcut.
                        bool altGr = mods.HasFlag(ShortcutModifiers.Control | ShortcutModifiers.Alt) &&
                            (GetAsyncKeyState(VK_RMENU) & 0x8000) != 0;
                        if (!altGr && (_suppressedKeys.Contains(vk) || TryHandleKeyDown(vk, mods)))
                        {
            ...
            else if (msg is WM_KEYUP or WM_SYSKEYUP)
            {
                var kbd = Marshal.PtrToStructure<KBDLLHOOKSTRUCT>(lParam);
                int vk = (int)kbd.vkCode;
                bool wasSuppressed = _suppressedKeys.Remove(vk);
                if (wasSuppressed && IsSlateForeground())
                {
