...
public sealed partial class MainWindow
{
        ...
        try
        {
            ...
            foreach (var accent in SlateTheme.AccentNames)
            {
                ...
                if (appearance)
                    ...
                    Check($"{accent} light preserves surface luminance hierarchy", SlateTheme.RelativeLuminance(palette.SurfaceRaised) > SlateTheme.RelativeLuminance(palette.Page) && SlateTheme.RelativeLuminance(palette.Page) > SlateTheme.RelativeLuminance(palette.Sidebar));
            }
                ...
                for (int i = 0; i < palettes.Count; i++)
                {
                    for (int j = i + 1; j < palettes.Count; j++)
                    {
                        ...
                        Check($"{nameA} vs {nameB} ({mode}) environmental surfaces differ by tier hierarchy",
                            ...
                            ColorDelta(palA.SurfaceRaised, palB.SurfaceRaised) >= 5 &&
                            ColorDelta(palA.SurfaceInteractive, palB.SurfaceInteractive) >= 6 &&
            ...
            Check("Amber is distinct from Clay in dark mode", clayDark.AccentPrimary != amberDark.AccentPrimary && ColorDelta(clayDark.AccentPrimary, amberDark.AccentPrimary) > 40 && ColorDelta(clayDark.SurfaceInteractive, amberDark.SurfaceInteractive) >= 6 && ColorDelta(clayDark.Page, amberDark.Page) >= 3);
            Check("Amber is distinct from Clay in light mode", clayLight.AccentPrimary != amberLight.AccentPrimary && ColorDelta(clayLight.AccentPrimary, amberLight.AccentPrimary) > 40 && ColorDelta(clayLight.SurfaceInteractive, amberLight.SurfaceInteractive) >= 6 && ColorDelta(clayLight.Page, amberLight.Page) >= 3);
            ...
            for (int i = 0; i < BrowserEngineService.MaximumPopupsPerBurst; i++) Check("Popup burst allowance " + (i + 1), _engineService.AllowPopup("https://popup-test.invalid", popupTime));
            Check("Popup bursts are rate limited", !_engineService.AllowPopup("https://popup-test.invalid", popupTime));
            for (int i = BrowserEngineService.MaximumPopupsPerBurst; i < BrowserEngineService.MaximumGlobalPopupsPerBurst; i++)
                Check("Global popup burst allowance " + (i + 1), _engineService.AllowPopup($"https://popup-{i}.invalid", popupTime));
            Check("Global popup bursts are rate limited", !_engineService.AllowPopup("https://popup-overflow.invalid", popupTime));
            _engineService.ClearRateLimits();
            ...
            Check("Location permission requires user initiation", PermissionPolicy(CoreWebView2PermissionKind.Geolocation)?.RequiresUserInitiation == true);
            for (int i = 0; i < BrowserEngineService.MaximumGlobalPermissionPromptsPerBurst; i++)
            ...
            var vsHtml = _navigationCoordinator.BuildViewSourceHtml(origin + "/one", "<html>source</html>");
            Check("BuildViewSourceHtml enforces strict Content Security Policy", vsHtml.Contains("Content-Security-Policy\" content=\"default-src 'none'; style-src 'unsafe-inline'"));
            ...
            foreach (var (name, show) in new (string, Func<Task>)[] { ("Palette", () => ShowPaletteAsync()), ("History", ShowHistoryAsync), ("Downloads", ShowDownloadsAsync), ("Settings", ShowSettingsAsync), ("Workspaces", ShowWorkspacesAsync), ("Site information", ShowSiteInfoAsync) })
            {
                ...
                if (name == "Settings")
                {
                    var headings = Descendants(_activeDialog!).OfType<TextBlock>().Select(t => t.Text).ToList();
                    Check("Settings groups daily-browser controls into seven sections", new[] { "Appearance", "Browsing", "Downloads", "Tabs", "Privacy / Data", "Profile", "About" }.All(headings.Contains));
    ...
    private static readonly List<string> _unexpectedListenerDetails = new();
    ...
    private static List<int> GetUnexpectedListeningPorts(int expectedPort)
    {
        ...
        _unexpectedListenerDetails.Clear();
            ...
            try
            {
                if (GetExtendedTcpTable(buffer, ref size, false, 2, 5, 0) == 0)
                {
                    ...
                    for (int i = 0; i < entries; i++)
                    {
                        ...
                        {
                            unexpected.Add(port);
                            _unexpectedListenerDetails.Add($"TCP4:{port} (pid={owningPid}, name={GetProcName(owningPid)})");
                        }
            ...
            try
            {
                if (GetExtendedTcpTable(buffer, ref size, false, 23, 5, 0) == 0)
                {
                    ...
                    for (int i = 0; i < entries; i++)
                    {
                        ...
                        {
                            unexpected.Add(port);
                            _unexpectedListenerDetails.Add($"TCP6:{port} (pid={owningPid}, name={GetProcName(owningPid)})");
                        }
            ...
            try
            {
                if (GetExtendedUdpTable(buffer, ref size, false, 2, 1, 0) == 0)
                {
                    ...
                    for (int i = 0; i < entries; i++)
                    {
                        ...
                        {
                            unexpected.Add(port);
                            _unexpectedListenerDetails.Add($"UDP4:{port} (pid={owningPid}, name={GetProcName(owningPid)})");
                        }
            ...
            try
            {
                if (GetExtendedUdpTable(buffer, ref size, false, 23, 1, 0) == 0)
                {
                    ...
                    for (int i = 0; i < entries; i++)
                    {
                        ...
                        {
                            unexpected.Add(port);
                            _unexpectedListenerDetails.Add($"UDP6:{port} (pid={owningPid}, name={GetProcName(owningPid)})");
                        }
